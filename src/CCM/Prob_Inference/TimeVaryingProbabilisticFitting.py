...
from WFStepFitness2D import WFStepFitness2D
...
def run_sim(parameters, target_data, return_clone_sizes=False):
        ...
        try:
            ...
            for loop in range(LOOP_LIMITS):
                model_type = parameters.get('model', 'linear')
                decay = parameters.get('decay', 0)
                if decay > 0:
                    if model_type == 'step':
                        s = WFStepFitness2D(p, a_intercept=fitness, t_time=decay)
...
# get_sim_mean_pl(test_p, TP53_MEAN)        # Commented this line out to stop the large data run which would take hours
