...
# -*- coding: utf-8 -*-
"""
Created on Fri 10 July 2026

@author: syedtariqishtiaq
"""

# This code will run a simulation for 2 timepoints but using the 
...
# This is what pyabc actually calls when it wants to test parameters
    a_intercept, b_slope, induction = parameters['a_intercept'], parameters['b_slope'], parameters['induction']
    # parameters is a dictionary so this calls certain keys in it
    try: 
        initial_grid, fitness_array, label_array = get_grid(a_intercept, induction, SHAPE_OF_GRID, CELLS)
...
if __name__ == "__main__":
    priors = Distribution(
        a_intercept = RV("uniform", 1, 1.5),
        b_slope = RV("uniform", 0, 0.005),        # Decay rate: 0 = no decay, 0.005 = fast decay, but fitness will never get catastrophically low
...
# very wide initial berth. RV is imported from pyabc.)
    ...
    # tells us to ignore the last 15 rows as we only want the first 
    # 2 timepoints. usecols says to only use
    ...
    db_path_first2 = ("sqlite:///" + "TP53First2_TimeVarying"+'_pyabc.db')
    # constructs address of where the database should live
    
    r = abc.new(db_path_first2)
    ...
    print("RunID:", r.id)
    hist_first2 = abc.run(minimum_epsilon=0.1, max_nr_populations=15)
    ...
    df, w = hist_first2.get_distribution() 
