...
# This code takes Ben Hall's code from FittingModels and adapt it for TP53 data instead, so it is searching a 
# grid for the best combination of parameters (fitness and induction) that models the actual data.

# At the end, I have plotted graphs to show loglikelihoods and best fit of the simulation to the data. 

# We can see that the simulation fits the early timepoints well, but not the later timepoints.
# In the file TimeVaryingProbabilisticFitting, I have taken this exact code and added in a search for an extra parameter: decay.
