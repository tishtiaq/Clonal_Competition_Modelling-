...
def get_sim_mean_pl(parameters, target_data):
    ...
    # The above function computes mean simulated takeover which gets used in the calculation of likelihood
        ...
        for loop in range(LOOP_LIMITS):
            if decay>0 :
                s = WFStepFitness(p, a_intercept=fitness, t_time=decay)
            else:
...
# Note that currently decayRange refers to t_time's boundaries not decayRange's (t_time is the time at which fitness drops to neutral)
def pseudoLikelihoodSweep3D(target_data, n=10, fitnessRange=(1, 2), inductionRange=(0.001, 0.03), decayRange=(50,  350)):
    fi, fg = rangeModifier(fitnessRange, n)
    ii, ig = rangeModifier(inductionRange, n)
    ...
    try:
        ...
        for loop in range(LOOP_LIMITS):
            if parameters.get('decay', 0) > 0:
                s = WFStepFitness(p, a_intercept=fitness, t_time=parameters['decay'])
            else:
                s = p.get_simulator()
            s.run_sim()
