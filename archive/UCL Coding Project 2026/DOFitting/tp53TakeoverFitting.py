...
# pyabc is built for approximate bayesian computation: technique for 
# estimating unknown parameters when you can simulate data but don't have
# a mathematial likelihood function
from pyabc.sampler import MulticoreEvalParallelSampler, SingleCoreSampler
from functools import partial, update_wrapper
import pandas as pd
import numpy as np
from scipy.stats import ks_2samp
# Kolmogorov-Smirnov test, compares whether two samples come from the same
# distribution
...
# This randomly scatters a number of mutant cells across the grid 
# (induction is the number of cells initially mutated) 
    count = 0
...
# Every induced mutant gets the same fitness and is labelled 1

def get_mutant_takeover(sim):
    mutants = sim.population_array.toarray()[1:]
    mutant_pop = mutants.sum(axis=0)
# Skips row 0 as they're wild type and sums every other row at each time
# point. This gives total mutant cell count over time. 
...
# Wants to track how far the simulation is from real results, and this 
# specific function uses the KS test
...
# Another tracker of how far simulation is from real results, measures the 
# proportion of variance in the target data that the simulation result 
# fails to explain
...
# Change above to FitnessParameters, TimeParameters
...
# Runs simulation up to 50 times, 
            if return_takeover:
            ...
            return ERROR_OBJECT
# Compares simulated outcome to the experimental data, returning a number
...
# This sets up our initial beliefs about the unknown parameters that we're
# trying to estimate
